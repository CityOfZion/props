...
describe('Basic IconDApp Test Suite', function () {
  ...
  beforeEach(async function () {
    await exec('neoxp batch -r ../test-env/initialize.batch')
    const {stdout} = await exec('neoxp contract get "Icon DApp by COZ & NNT" -i ../test-env/default.neo-express')

    const neoxpContract = JSON.parse(stdout)[0]
    scriptHash = neoxpContract.hash
    spawn('neoxp', ['run', '-i', '../test-env/default.neo-express', '-s', '1'], {})
    await wait(800)

    const network = JSON.parse(fs.readFileSync('../test-env/default.neo-express').toString())
  ...
  afterEach('Tear down', async function () {
    await exec('neoxp stop -i ../test-env/default.neo-express')
    return true
  })
  ...
  it('Tests getProperties empty', async () => {
    ...
    assert(Object.keys(resp).length === 0)
  })
  ...
  it('Tests testSetOwnership', async () => {
    ...
    const {stdout} = await exec('neoxp contract get "Ownership" -i ../test-env/default.neo-express')
