...
class CharacterAPI {
        ...
        if (res[0] && res[0].value) {
            res[0].value.forEach((entry) => {
                ...
                switch (key) {
                    ...
                    case "token_id":
                        rawValue = neon_js_1.u.base642hex(entry.value.value);
                        character.tokenId = parseInt(neon_js_1.u.reverseHex(rawValue), 16);
                        break;
                }
            });
        }
    ...
    static async getAuthorizedAddresses(node, networkMagic, contractHash) {
    ...
    static async setAuthorizedAddress(node, networkMagic, contractHash, address, authorized, signer) {
        const method = "setAuthorizedAddress";
        const params = [
            neon_js_1.sc.ContractParam.hash160(address),
            neon_js_1.sc.ContractParam.boolean(authorized)
        ];
        try {
            const res = await interface_1.NeoInterface.publishInvoke(node, networkMagic, contractHash, method, params, signer);
            if (res === undefined || res.length === 0) {
