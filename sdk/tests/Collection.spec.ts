...
describe('Basic Collection Test Suite', function () {
  ...
  beforeEach(async function () {
    ...
    const {stdout} = await exec('neoxp contract get "Collection" -i ../default.neo-express')
  ...
  it("Tests popping values from a very large collection", async() => {
    ...
    const collectionLength = await collection.getCollectionLength({collectionId: cid})
  ...
  it("Tests uniformly sampling from the range of values in a collection", async() => {
    ...
    const txids: string[] = []
    for (let i = 0; i < runSize; i++) {
        const res = await collection.sampleFromCollection({collectionId: cid, samples: 1})
        txids.push(res)
    }
  ...
  it("Tests picking 9 of the 10 samples multiple times without repeating", async() => {
    ...
    const txids: string[] = []
    for (let i = 0; i < 20; i++) {
       const txid = await collection.sampleFromRuntimeCollection({values:options, samples: 9, pick: true})
       txids.push(txid)
    }
    await wait(TIME_CONSTANT)
  ...
  it('Tests rejecting a runtime sampling because the picks count is larger than the array length', async() => {
    ...
    try {
        await collection.sampleFromRuntimeCollection({values: options, samples: 12, pick: true})
