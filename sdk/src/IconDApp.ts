import type { IconProperties, SmartContractConfig } from './types'
import {
...
export class IconDApp {
  ...
  async getOwner(): Promise<string> {
    const res = await this.config.invoker.testInvoke({
      invocations: [IconDApp.buildGetOwnerInvocation(this.config.scriptHash)],
      signers: []
    })
  ...
  async getName(): Promise<string> {
    const res = await this.config.invoker.testInvoke({
      invocations: [IconDApp.buildNameInvocation(this.config.scriptHash)],
      signers: []
    })
   ...
   */
  async testUpdateProperty(options: {
    propertyName: string
    description: string
  }): Promise<boolean> {
    const res = await this.config.invoker.testInvoke({
  ...
  async getMetaData(options: {
    scriptHash: string
  }): Promise<IconProperties & Record<string, any>> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        IconDApp.buildGetMetaDataInvocation(this.config.scriptHash, {
          scriptHash: options.scriptHash
        })
      ],
      signers: []
   ...
   */
  async getMultipleMetaData(options: {
    contractHashes: string[]
  }): Promise<Map<string, Map<string, string>>[]> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        IconDApp.buildGetMultipleMetaDataInvocation(this.config.scriptHash, {
          contractHashes: options.contractHashes
        })
      ],
      signers: []
    })
   ...
   */
  async testSetContractParent(options: {
    childHash: string
    parentHash: string
  }): Promise<boolean> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        IconDApp.buildSetContractParentInvocation(this.config.scriptHash, {
          childHash: options.childHash,
   ...
   * Sets the parent of a smart contract, children will have the same metadata as the parent. (Admin and deployer only)
   */
  async setContractParent(options: {
    childHash: string
    parentHash: string
  }): Promise<string> {
    return await this.config.invoker.invokeFunction({
  ...
  async getContractParent(options: { childHash: string }): Promise<string> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        IconDApp.buildGetContractParentInvocation(this.config.scriptHash, {
          childHash: options.childHash
        })
      ],
      signers: []
   ...
   */
  async testSetOwnership(options: {
    scriptHash: string
    sender: string
  }): Promise<boolean> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        IconDApp.buildSetOwnershipInvocation(this.config.scriptHash, {
          scriptHash: options.scriptHash,
  ...
  static buildAddPropertyInvocation(
    scriptHash: string,
    parser: Neo3Parser,
    params: { propertyName: string; description: string }
  ): ContractInvocation {
  ...
  static buildUpdatePropertyInvocation(
    scriptHash: string,
    parser: Neo3Parser,
    params: { propertyName: string; description: string }
  ): ContractInvocation {
  ...
  static buildSetMetaDataInvocation(
    scriptHash: string,
    parser: Neo3Parser,
    params: { scriptHash: string; propertyName: string; value: string }
  ): ContractInvocation {
  ...
  static buildGetMetaDataInvocation(
    scriptHash: string,
  ...
  static buildGetMultipleMetaDataInvocation(
    scriptHash: string,
    ...
    return {
      ...
      args: [
        {
          type: 'Array',
          value: params.contractHashes.map(hash => ({
            type: 'Hash160',
            value: hash
          }))
        }
  ...
  static buildSetContractParentInvocation(
    scriptHash: string,
    params: { childHash: string; parentHash: string }
  ): ContractInvocation {
    return {
      scriptHash,
  ...
  static buildGetContractParentInvocation(
    scriptHash: string,
  ...
  static buildSetOwnershipInvocation(
    scriptHash: string,
    params: { scriptHash: string; sender: string }
  ): ContractInvocation {
    return {
      scriptHash,
