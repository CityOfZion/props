...
export async function txDidComplete(
  node: string,
  txid: string,
  ...
  if (showStats) {
    console.log(
      'gas consumed: ',
      parseInt(tx.executions[0].gasconsumed) / 10 ** 8
    )
...
export function chiSquaredFunction(samples: string[]): number {
  const bins = {}
...
export function capFirst(string_: string) {
