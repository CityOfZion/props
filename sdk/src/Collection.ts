import type { CollectionType, SmartContractConfig } from './types'
...
export class Collection {
  ...
  async createCollection(options: {
    ...
    const res = await this.config.invoker.invokeFunction({
      invocations: [
        Collection.buildCreateCollectionInvocation(this.config.scriptHash, {
   ...
   */
  async getCollectionJSON(options: {
    collectionId: number
  }): Promise<CollectionType> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildGetCollectionJSONInvocation(this.config.scriptHash, {
  ...
  async getCollection(options: { collectionId: number }): Promise<string> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildGetCollectionInvocation(this.config.scriptHash, {
          collectionId: options.collectionId
        })
      ],
      signers: []
   ...
   */
  async getCollectionElement(options: {
    collectionId: number
    index: number
  }): Promise<string> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildGetCollectionElementInvocation(this.config.scriptHash, {
          collectionId: options.collectionId,
          index: options.index
        })
      ],
   ...
   */
  async getCollectionLength(options: {
    collectionId: number
  }): Promise<number> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildGetCollectionLengthInvocation(this.config.scriptHash, {
          collectionId: options.collectionId
        })
      ],
      signers: []
   ...
   */
  async getCollectionValues(options: {
    collectionId: number
  }): Promise<(string | number)[]> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildGetCollectionValuesInvocation(this.config.scriptHash, {
          collectionId: options.collectionId
        })
      ],
      signers: []
   ...
   */
  async mapBytesOntoCollection(options: {
    collectionId: number
    entropy: string
  }): Promise<string> {
    const res = await this.config.invoker.testInvoke({
   ...
   * testing.
   */
  async sampleFromCollection(options: {
    collectionId: number
    samples: number
  }): Promise<string> {
    return await this.config.invoker.invokeFunction({
      invocations: [
        Collection.buildSampleFromCollectionInvocation(this.config.scriptHash, {
          collectionId: options.collectionId,
   ...
   * testing.
   */
  async sampleFromRuntimeCollection(options: {
    values: string[]
    samples: number
    pick: boolean
  }): Promise<string> {
  ...
  async totalCollections(): Promise<number> {
    const res = await this.config.invoker.testInvoke({
      invocations: [
        Collection.buildTotalCollectionsInvocation(this.config.scriptHash)
      ],
      signers: []
    })
  ...
  static buildCreateCollectionInvocation(
    scriptHash: string,
    params: {
      description: string
      collectionType: string
  ...
  static buildGetCollectionJSONInvocation(
    scriptHash: string,
  ...
  static buildGetCollectionInvocation(
    scriptHash: string,
    params: { collectionId: number }
  ): ContractInvocation {
    return {
  ...
  static buildGetCollectionElementInvocation(
    scriptHash: string,
    params: { collectionId: number; index: number }
  ): ContractInvocation {
    return {
      scriptHash,
  ...
  static buildGetCollectionLengthInvocation(
    scriptHash: string,
    params: { collectionId: number }
  ): ContractInvocation {
    return {
  ...
  static buildGetCollectionValuesInvocation(
    scriptHash: string,
    params: { collectionId: number }
  ): ContractInvocation {
    return {
  ...
  static buildMapBytesOntoCollectionInvocation(
    scriptHash: string,
    params: { collectionId: number; entropy: string }
  ): ContractInvocation {
    return {
      scriptHash,
  ...
  static buildSampleFromCollectionInvocation(
    scriptHash: string,
    params: { collectionId: number; samples: number }
  ): ContractInvocation {
    return {
      scriptHash,
  ...
  static buildSampleFromRuntimeCollectionInvocation(
    scriptHash: string,
    params: { values: string[]; samples: number; pick: boolean }
  ): ContractInvocation {
    return {
      scriptHash,
      ...
      args: [
        {
          type: 'Array',
          value: params.values.map(value => ({ type: 'String', value: value }))
        },
        { type: 'Integer', value: params.samples },
  ...
  static buildTotalCollectionsInvocation(
    scriptHash: string
  ): ContractInvocation {
    return {
      scriptHash,
