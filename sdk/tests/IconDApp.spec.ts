...
describe('Basic IconDApp Test Suite', function () {
  ...
  beforeEach(async function () {
    ...
    await exec(
      'neoxp batch -i ../../../../default.neo-express ./tests/batch_files/icon_dapp/initialize.batch'
    )
    const { stdout } = await exec(
      'neoxp contract get "Icon DApp by COZ & NNT" -i ../default.neo-express'
    )
  ...
  it('Tests testAddProperty', async () => {
    ...
    await assert.rejects(
      async () =>
        await iconDapp.testAddProperty({
          propertyName: 'prop1',
  ...
  it('Tests testUpdateProperty', async () => {
    ...
    await assert.rejects(
      async () =>
        await iconDapp.testUpdateProperty({
          propertyName: 'prop1',
          description: 'description1'
        }),
      /No authorization/
  ...
  it('Tests testSetMetaData', async () => {
    ...
    await assert.rejects(
      async () =>
        await iconDapp.testSetMetaData({
  ...
  it('Tests setMetaData invalid length', async () => {
    const owner = wallets.find((wallet: any) => wallet.name === 'coz')
    const iconDapp = await getSdk(owner.account)
  ...
  it('Tests getMultipleMetaData', async () => {
    ...
    const resp: any = await iconDapp.getMultipleMetaData({
      contractHashes: [
        '0x14d91cd393bc06c571b966df1cc59c0115bdb59c',
        '0xd2a4cff31913016155e38e474a2c06d08be276cf'
      ]
    })

    assert.equal(
      resp['0x14d91cd393bc06c571b966df1cc59c0115bdb59c'].prop1,
      'https://www.google.com/'
    )
    assert.equal(
      resp['0xd2a4cff31913016155e38e474a2c06d08be276cf'].prop1,
  ...
  it('Tests setContractParent with same hash for child and parent', async () => {
    ...
    await assert.rejects(
      async () =>
        await iconDapp.setContractParent({
          childHash: '0x49cf4e5378ffcd4dec034fd98a174c5491e395e2',
  ...
  it('Tests testSetOwnership', async () => {
    ...
    const { stdout } = await exec(
      'neoxp contract get "Ownership" -i ../default.neo-express'
    )
  ...
  it('Tests setOwnership', async () => {
    ...
    let contractMetadata = await iconDappUser.getMetaData({
      scriptHash: contractScriptHash
    })
    assert(
      contractMetadata[propertyName] === undefined,
      'Property value should not have been added'
    )
    ...
    contractMetadata = await iconDappUser.getMetaData({
      scriptHash: contractScriptHash
    })
    assert(
      contractMetadata[propertyName] === value,
      'Property value should have been added'
    )
  })
})
