...
export function buildRandBetweenInvocation(scriptHash: string, params: {start: number, end: number}): ContractInvocation{
  return {
    scriptHash,
...
export function buildMapBytesOntoRangeInvocation(scriptHash: string, params: {start: number, end: number, entropy: string}): ContractInvocation{
  return {
    scriptHash,
...
export function buildRollDieInvocation(scriptHash: string, params: {die: string}): ContractInvocation{
  return {
    scriptHash,
    operation: 'roll_die',
    args: [
...
export function buildRollDieWithEntropyInvocation(scriptHash: string, parser: Neo3Parser, params: {die: string, precision: number, }): ContractInvocation{
  return {
    scriptHash,
    operation: 'roll_die',
    args: [
...
export class Dice {
   ...
   */
    ...
    return await this.config.invoker.invokeFunction({
      invocations: [
        buildRollDieInvocation(this.config.scriptHash, {die: options.die})
      ],
      signers: [],
    })
	}
